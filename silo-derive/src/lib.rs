        ...
        impl silo::FromRow for #partial_name {
            fn try_from_row(
                ...
                let Some(Some(#primary_key_field)) = #primary_key_field else {
                    eprintln!("Primary key field was none!!!");
        ...
        quote! {
            ...
            fn try_from_row(
                ...
                #(
                    ...
                    let #field_names = <<#field_types as silo::HasPartialRepresentation>::Partial>::try_from_row(string_storage, Some(actual_column_name), row, connection).unwrap_or_default();
                )*
                Some(Self {
                    #( #field_names),*
                })
